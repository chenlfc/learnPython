...
def get_number_aliens_x(ai_settings, alien_width):
    ...
    # print('ailien_width = ', alien_width)
...
def check_events(ai_settings, screen, stats, score_board, ship, aliens,
                 bullets, play_button):
    """响应按键和鼠标事件"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ai_settings.save_to_json()
            sys.exit()
...
def check_play_button(ai_settings, screen, stats, score_board, play_button,
    ...
    if button_clicked and not stats.game_active:
        new_game(ai_settings, screen, stats, score_board, ship, aliens,
                 bullets)


def new_game(ai_settings, screen, stats, score_board, ship, aliens, bullets):
    """重新开始游戏"""
    # 重置游戏设置
...
def check_keydown_events(event, ai_settings, screen, stats, score_board, ship,
                         aliens, bullets):
    """响应按键"""
...
def check_bullet_alien_collisions(ai_settings, screen, stats, score_board,
    ...
    if collisions:
        for this_aliens in collisions.values():
            stats.score += ai_settings.alien_points * len(this_aliens)
            score_board.prep_score()
...
def update_aliens(ai_settings, screen, stats, score_board, ship, aliens,
    ...
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(ai_settings, stats, screen, score_board, ship, aliens,
                 bullets)
    check_aliens_bottom(ai_settings, stats, screen, score_board, ship, aliens,
                        bullets)
...
def check_aliens_bottom(ai_settings, stats, screen, score_board, ship, aliens,
                        bullets):
    """检查是否有外星人到达了屏幕底端"""
